...
class Guard:
    ...
    def __init__(self):
        ...
        self.min_frames_non_detection = 1
    ...
    def addMember(self, img, name):
        ...
        self.members = self.members.append(pd.DataFrame([[member_id, name, 0]], columns=["id", "name", "instances"]),
                                           ignore_index=True)
        print(self.members)
        imwrite(path_member_images + "/" + str(self.members.id[member_id]) + "-" + str(
            self.members.name[member_id]) + ".png", img)
        return member_id
    ...
    def addLabeledData(self, member_id, face_encoding):
        ...
        self.classifier.fit(self.labeled_db[:, 1:],
                            np.array(self.labeled_db[:, :1].reshape(self.labeled_db.shape[0]), dtype=int))
        self.save()
    ...
    def humanDetected(self):
        if self.frames_detecting >= self.min_frames_detection and self.frames_not_detecting >= self.min_frames_non_detection:
