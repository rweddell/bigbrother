	...
	if guard.humanDetected():
		...
		if len(guard.knownMembers()) == 0:
			...
			if img is not None:
				...
				bigbrother.post_message("I'm sorry, I'm new here, could you identify this person that just came in? \n please respond in this channel with the person's name")
			...
			if face_encoding is not None:
				...
				if prediction.get("error") == "not enough data":
					img = frame.getFace(0)
					bigbrother.post_image("guest", img)
					bigbrother.post_message("ok, I'm still learning, could you help me identify this person?  \n please respond in this channel with the person's name")
					...
					if len(guard.knownMembers()) == 1:
						...
						if confirmation.get("response") is None:
							if confirmation.get("text") is None:
								...
								bigbrother.post_message("sorry, I didn't understand your answer could you be more clear like saying yes or no?")
								confirmation = guard.getConfirmation()
								if confirmation.get("response") is None:
									if confirmation.get("text") is None:
										bigbrother.post_message("oh never mind :face_with_rolling_eyes:")
									else:	
										bigbrother.post_message("oh... this is obviously to hard for you, never mind :face_with_rolling_eyes:")
								else:
									if confirmation.get("response"):
										bigbrother.post_message("great! thanks")
										guard.addLabeledData(prediction.get("id"), face_encoding)
									else:	
										bigbrother.post_message("oh ok, sorry I'm still getting to know you guys \n who is it then?")
										if guard.getLabel(img, face_encoding):
											bigbrother.post_message("ok! I'll try to remember, I'll get better soon don't worry")
										else:
						...
						if prediction.get("probability") > 0.9:
							...
							if (ts - last_ts) > 5400: # 15 minutes 
								bigbrother.post_message(prediction.get("name")+" just entered the office :slightly_smiling_face:")
						else:
							if prediction.get("probability") > 0.7:
								...
								img = frame.getFace(0)
								bigbrother.post_image("guest", img)
								bigbrother.post_message("well, I don't know who this is, could you help me identify this person?  \n please respond in this channel with the person's name")
								if guard.getLabel(img, face_encoding):
									bigbrother.post_message("Thanks, I'm learning, don't worry, I'll get better at this soon")
								else:
									bigbrother.post_message("Never mind, I guess you're busy")
